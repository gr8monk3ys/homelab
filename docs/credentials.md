...
These are **not** generated by `scripts/generate-secrets.sh`. You create them
...
`scripts/secrets-check.sh` compares the `upsert_secret` calls in
`scripts/generate-secrets.sh` with every ExternalSecret `remoteRef.key` under
`kubernetes/` and `helm/`, plus the `kubectl get secret -n secrets <name>`
recipes in scripts and docs. A generated secret nothing reads, or a consumed
name nothing generates, fails the check. `--list` prints the sets.
